...
export default function Archive() {
  ...
  return (
    <motion.section initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <div>
        <div className="font-mono text-[12px] tracking-[.22em] text-cyan">ARSIP BUKTI · KONSEP TERKUMPUL</div>
        <h2 className="mt-2.5 font-display text-[clamp(24px,4vw,40px)] font-bold">PERPUSTAKAAN KEADILAN AI</h2>
      </div>

      <div>
        <div className="mb-3 font-mono text-[12px] tracking-[.18em] text-gold">GLOSARIUM</div>
      ...
      <div>
        <div className="mb-3 font-mono text-[12px] tracking-[.18em] text-gold">BERKAS TERBUKA</div>
        <div className="grid gap-3.5 sm:grid-cols-2 xl:grid-cols-3">
          {entries.length === 0 && (
            <div className="border border-dashed border-haze/30 p-6 font-mono text-[12px] leading-loose text-haze/70">
          ...
          {entries.map((e) => (
            <div key={`${e.code}-${e.id}`} className="border border-cyan/20 bg-[#061426]/60 p-[18px]">
              <div className="font-mono text-[12px] tracking-[.16em] text-gold">{e.code} · {e.type}</div>
