    ...
    <motion.section initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <div className="flex min-w-0 flex-col gap-[18px]">
        <div className="flex flex-wrap items-baseline justify-between gap-3.5 border-b border-cyan/20 pb-3.5">
          <div>
            <div className="font-mono text-[12px] tracking-[.2em] text-cyan">{gameCase.room}</div>
          ...
          <div className="flex flex-wrap gap-2">
            {gameCase.principles.map((pr) => (
              <span key={pr} className="border border-gold/35 px-2.5 py-1.5 font-mono text-[12px] tracking-[.14em] text-gold">{pr}</span>
            ))}
          </div>
        </div>
          ...
          {PHASES.map((ph) => {
            ...
            return (
              <button key={ph.id} disabled={blocked} onClick={() => g.setPhase(ph.id)}
                ...
                className={`min-h-[44px] border px-3.5 py-2.5 font-mono text-[12px] tracking-[.14em] ${active ? "border-gold bg-gold text-navy" : blocked ? "cursor-not-allowed border-haze/20 text-haze/40" : "border-haze/25 text-haze hover:border-cyan hover:text-cyan"}`}>
                {ph.label}
              </button>
            );
          })}
        ...
        {g.phase === "brief" && dlg.line && (
          <div className="border border-cyan/20 bg-navy/60 p-6 md:p-8">
            <div className="font-mono text-[12px] tracking-[.2em] text-gold">BRIEFING KASUS</div>
            ...
            <div className="mb-5 grid gap-2.5 sm:grid-cols-3">
              {gameCase.background.map((f) => (
                <div key={f.label} className="border-l-2 border-cyan bg-cyan/[.05] px-3.5 py-2.5">
                  <div className="font-mono text-[12px] tracking-[.16em] text-haze">{f.label}</div>
            ...
            <div className="mb-6 grid gap-3 border border-haze/15 p-4 md:grid-cols-2">
              <div>
                <div className="font-mono text-[12px] tracking-[.16em] text-cyan">KEPUTUSAN AI · {ai.system}</div>
              ...
              <div className="border-l border-haze/15 pl-4">
                <div className="font-mono text-[12px] tracking-[.16em] text-gold">MANFAAT SISTEM INI</div>
                <p className="mt-2 text-[13.5px] leading-relaxed text-[#e3ebf3]">{ai.benefit}</p>
                <p className="mt-1.5 font-mono text-[12px] leading-relaxed text-haze">AI tidak selalu salah. Tugasmu menilai, bukan menyalahkan.</p>
              </div>
            </div>
        ...
        {g.phase === "analysis" && (
          <div className="border border-gold/25 bg-navy/60 p-6 md:p-7">
            <div className="font-mono text-[12px] tracking-[.2em] text-gold">MEJA ANALISIS · RANGKAI BUKTI</div>
            <p className="mb-1.5 mt-3 text-[15.5px] leading-relaxed text-[#e3ebf3]">{gameCase.analysis.prompt}</p>
            <div className="mb-5 font-mono text-[12px] text-haze">DIPILIH {p.picks.length} / {gameCase.analysis.pick}</div>
              ...
              {scanned.map((e) => {
                ...
                return (
                  <button key={e.id} onClick={() => g.togglePick(e.id, gameCase.analysis.pick)}
                    ...
                    <span className="font-mono text-[12px] tracking-[.14em] opacity-70">{e.type}</span>
            ...
            {p.analysisSubmitted && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-5 border border-cyan/30 bg-cyan/[.06] p-[18px]">
                <div className="font-mono text-[12px] tracking-[.18em] text-cyan">RANTAI KESIMPULAN</div>
        ...
        {g.phase === "verdict" && (
          <div className="flex flex-col gap-4 border border-gold/30 bg-gold/[.05] p-6">
            <div className="font-mono text-[12px] tracking-[.2em] text-gold">SIAP MEMUTUS</div>
      ...
      <aside className="sticky top-5 flex flex-col gap-[18px] border border-haze/20 bg-[#061222]/70 p-[18px]">
        <div>
          <div className="font-mono text-[12px] tracking-[.2em] text-gold">DOSIR ANALIS</div>
          <div className="mt-2 font-display text-[14px]">{playerName}</div>
        </div>
        <div>
          <div className="mb-2.5 font-mono text-[12px] tracking-[.2em] text-cyan">BUKTI TERKUMPUL · {scanned.length}/{evidence.length}</div>
          <div className="flex flex-col gap-1.5">
            {scanned.length === 0 && <div className="font-mono text-[12px] leading-relaxed text-haze/70">Belum ada bukti dipindai.</div>}
            {scanned.map((e) => (
              <div key={e.id} className="flex items-baseline gap-2 border-l-2 border-cyan/50 pl-2.5 text-[12px] text-[#d7e2ee]">
                <span className="font-mono text-[12px] text-cyan">{e.impact === "HIGH" ? "★" : "•"}</span>
                <span>{e.title}</span>
              </div>
            ))}
          </div>
        ...
        <div>
          <div className="mb-2.5 font-mono text-[12px] tracking-[.2em] text-gold">WAWASAN TERBUKA</div>
          <div className="flex flex-col gap-1.5">
            {insights.length === 0 && <div className="font-mono text-[12px] leading-relaxed text-haze/70">Interogasi LUMA untuk membuka wawasan.</div>}
            {insights.map((m, i) => (
              <div key={i} className="border border-gold/25 bg-gold/[.09] px-2.5 py-2 text-[12px] leading-snug text-white">{m.unlock}</div>
            ))}
          </div>
        </div>
        <button onClick={() => navigate("/cases")} className="mt-auto min-h-[44px] border border-haze/25 py-2.5 font-mono text-[12px] tracking-[.16em] text-haze hover:text-white">
