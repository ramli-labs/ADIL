...
export default function CaseSelection() {
  ...
  return (
    <motion.section initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <div>
        <div className="font-mono text-[12px] tracking-[.22em] text-cyan">PILIH BERKAS SIDANG</div>
          ...
          return (
            <motion.div key={c.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.07 }}
              ...
              <div className="flex items-center justify-between gap-2.5">
                ...
                <span className={`border px-2 py-1 font-mono text-[12px] tracking-[.14em] ${record ? "border-gold/50 text-gold" : unlocked ? "border-cyan/45 text-cyan" : "border-haze/25 text-haze/70"}`}>
              ...
              <div className="font-mono text-[12px] text-haze">{c.title_en}</div>
              <div className="my-4 flex flex-wrap gap-2">
                <span className="border border-cyan/30 px-2 py-1 font-mono text-[12px] tracking-[.12em] text-cyan">{c.topic}</span>
                <span className="border border-haze/25 px-2 py-1 font-mono text-[12px] tracking-[.12em] text-haze">{c.difficulty}</span>
                <span className="border border-haze/25 px-2 py-1 font-mono text-[12px] tracking-[.12em] text-haze">{getEvidence(c.evidence_set).length} BERKAS</span>
              </div>
              <p className="mb-4 text-pretty text-[13.5px] leading-relaxed text-haze">{c.briefing.slice(0, 120)}…</p>
              <div className="mb-5 font-mono text-[12px] leading-relaxed text-haze/70">{learning?.duration} · {learning?.principle}</div>
              <button disabled={!unlocked} onClick={() => { startCase(c.id); navigate(`/case/${c.id}`); }}
                className={`mt-auto flex items-center justify-center gap-2 py-3.5 font-display text-[12px] font-bold tracking-[.14em] ${unlocked ? "bg-cyan text-navy hover:brightness-110" : "cursor-not-allowed bg-haze/10 text-haze/60"}`}>
