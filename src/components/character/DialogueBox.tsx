...
export default function DialogueBox({
  ...
  return (
    <div className="border border-gold/30 bg-navy/80 p-6 backdrop-blur md:p-8">
      <div className="mb-3.5 font-mono text-[9px] tracking-[.2em] text-gold">TRANSKRIP SUARA · SUBTITLE AKTIF</div>
      ...
      <div className="mt-6 flex flex-wrap items-center gap-4">
        ...
        <button onClick={onSkip} className="border border-haze/30 px-5 py-3.5 font-mono text-[10px] tracking-[.16em] text-haze hover:text-white">
          LEWATI DIALOG
        </button>
        <span className="font-mono text-[10px] text-haze">{index + 1} / {total}</span>
      </div>
    </div>
  );
}
