    ...
    <header className="glass relative z-20 flex flex-wrap items-center gap-4 border-b border-cyan/15 px-6 py-3.5">
      <Link to="/" className="flex items-center gap-3">
        ...
        <span className="flex flex-col leading-none">
          ...
          <span className="mt-1 font-mono text-[8.5px] tracking-[.22em] text-haze">{config.game.subtitle.toUpperCase()}</span>
        </span>
      </Link>
      <div className="flex-1" />
      <span className="font-mono text-[10px] tracking-[.16em] text-haze">{context}</span>
      <div className="flex items-center gap-2.5 border border-gold/35 bg-gold/[.07] px-3 py-1.5">
        <span className="font-mono text-[9px] tracking-[.18em] text-gold">JUSTICE SCORE</span>
        <span className="font-display text-[15px] font-bold">{justiceScore}</span>
      </div>
      <Link to="/teacher" className="flex items-center gap-2 border border-nara/35 px-3 py-2 font-mono text-[9.5px] tracking-[.16em] text-nara hover:bg-nara/10">
        <GraduationCap className="h-3.5 w-3.5" /> GURU
      </Link>
      <button onClick={toggleAudio} className="flex items-center gap-2 border border-haze/30 px-3 py-2 font-mono text-[9.5px] tracking-[.16em] text-haze hover:border-cyan hover:text-cyan">
