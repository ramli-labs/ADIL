  ...
  return (
    <motion.section initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <span className="font-mono text-[9px] tracking-[.22em] text-nara">RINGKASAN DISKUSI GURU</span>
        <span className="ml-auto font-mono text-[9px] tracking-[.16em] text-haze">{learning.duration} · {gameCase.case_code}</span>
      </div>

      <div className="mt-5 grid gap-5 lg:grid-cols-2">
        <div>
          <h4 className="font-mono text-[9px] tracking-[.18em] text-gold">ISU KASUS</h4>
          ...
          <h4 className="mt-5 font-mono text-[9px] tracking-[.18em] text-gold">TUJUAN PEMBELAJARAN</h4>
          <ul className="mt-2 space-y-1.5 text-[13.5px] leading-relaxed text-[#e3ebf3]">
            {learning.objectives.map((o) => <li key={o} className="border-l-2 border-cyan/40 pl-3">{o}</li>)}
          </ul>

          <h4 className="mt-5 font-mono text-[9px] tracking-[.18em] text-gold">PRINSIP ETIKA AI</h4>
          <p className="mt-2 text-[13.5px] leading-relaxed text-white">{learning.principle} — {learning.takeaway}</p>
        </div>

        <div>
          <h4 className="font-mono text-[9px] tracking-[.18em] text-gold">PENALARAN SISWA (SESI INI)</h4>
          <div className="mt-2 grid grid-cols-2 gap-2 font-mono text-[10.5px] text-haze">
          ...
          <h4 className="mt-5 font-mono text-[9px] tracking-[.18em] text-gold">PERTANYAAN REFLEKSI</h4>
          ...
          <h4 className="mt-5 font-mono text-[9px] tracking-[.18em] text-gold">AKTIVITAS DISKUSI KELAS</h4>
          <ul className="mt-2 space-y-1.5 text-[13.5px] leading-relaxed text-[#e3ebf3]">
            {learning.discussion_prompts.map((p) => <li key={p} className="border-l-2 border-nara/40 pl-3">{p}</li>)}
          </ul>

          <h4 className="mt-5 font-mono text-[9px] tracking-[.18em] text-gold">TUGAS REFLEKSI</h4>
      ...
      <div className="mt-5 border-t border-nara/25 pt-4">
        <h4 className="font-mono text-[9px] tracking-[.18em] text-gold">MISKONSEPSI YANG SERING MUNCUL</h4>
