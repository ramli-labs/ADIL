  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-2">
        ...
        <span className="border px-1.5 py-0.5 font-mono text-[8.5px] tracking-[.14em]" style={{ color, borderColor: `${color}55` }}>
      ...
      {scanned ? (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-3.5 border-t border-dashed border-cyan/30 pt-3">
          <div className="font-mono text-[8.5px] tracking-[.16em] text-cyan">TEMUAN</div>
          <p className="mb-0 mt-1.5 text-[13.5px] leading-snug text-white">{evidence.finding}</p>
          {showConcept && <p className="mb-0 mt-2.5 font-mono text-[10.5px] leading-relaxed text-gold">KONSEP: {evidence.learningConcept}</p>}
        </motion.div>
      ) : (
